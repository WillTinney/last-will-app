...
feature 'Other Recipient Create Page', js: false do
  let(:user) { FactoryGirl.create(:user) }

  before do
    login_as(user, :scope => :user)
    visit "/users/#{user.id}/assignees/new.Other"
  end
  ...
  it 'relationship is other' do
    expect(find('#assignee_relationship', :visible => false).value).to_not eq('Partner' || 'Child')
  end
  ...
  scenario 'user can add recipient details' do
