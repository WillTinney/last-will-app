...
feature 'Approver Create Page', js: false do
  let(:user) { FactoryGirl.create(:user) }

  before do
    login_as(user, :scope => :user)
    visit "/users/#{user.id}/assignees/new.Approver"
  end
  ...
  it 'type is approver' do
    expect(find('#set_assignee_type', :visible => false).value).to eq('Approver')
  end
  ...
  scenario 'user can add approver details' do
